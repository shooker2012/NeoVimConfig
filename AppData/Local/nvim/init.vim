...
" inoremap <expr> <Tab> coc#pum#visible() ? coc#pum#next(1) : <Plug>SuperTabForward
" inoremap <expr> <S-Tab> coc#pum#visible() ? prev(1) : <Plug>SuperTabBackward
...
function! s:CopyFilePath(...)
	let res = ""
	if (a:0 == 0 || a:1 == 'p' || a:1 == '')
		execute 'let res='..escape(expand("%:p:S"), "\\")
	else
		execute 'let res='..escape(expand("%:S"), "\\")
	endif

	if (a:0 > 1)
		let res=substitute(res, "\\", a:2, 'g')
		let ret=substitute(res, "/", a:2, 'g')
	endif

	let @*=res
endfunction

command! -nargs=* CFP call <SID>CopyFilePath(<f-args>)

" autoload
set autoread
autocmd FocusGained,BufEnter,CursorHold,CursorHoldI * checktime
autocmd FileChangedShellPost * echohl WarningMsg | echo "File changed on disk. Buff reloaded" | echohl None